...
void AirConditionOn(int val) //zzx
{
    switch(val){
        case 1:
...
void *AppDemoAht20(char *param)
{
    ...
    while (1) {
        ...
        //对比温度并降温
        //printf("[zzx] zhunbei jiang wen\n");
        if(getBeginTemp() == 1){
            changeSetValue();
            printf("setTemp = %f\n",setTemp);
            if(GetAhtSensorValue(1) - setTemp > 2){
                AirConditionOn(1);
                printf("[zzx] colding~~~\n");
            }
            else if(GetAhtSensorValue(1) - setTemp < -2){
                AirConditionOn(0);
                printf("[zzx] hotting~~~\n");
            }
            else{
                AirConditionOn(-1);
                printf("[zzx] arrive at setting Temperature\n");
            }
        }
...
void changeSetValue(){
    ...
    switch(val){
        case 12:
        setTemp = GetAhtSensorValue(1);  //设置温度为当前温度
        break;
        case 1:
        setTemp = 12;
        break;
...
static void StartAht20Task(void)
{
    ...
    attr.priority = AHT20_DEMO_TASK_PRIORITY + 5;
