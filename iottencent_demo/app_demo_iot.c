...
#include "ohos_init.h"
#include "cmsis_os2.h"
#include "..\environment_demo\app_demo_environment.h"
#include "..\environment_demo\app_demo_aht20.h"
#include "..\uart_demo\uart_control.h"
#include "..\environment_demo\HX711.h"
...
/* CPU Sleep time Set */
unsigned int TaskMsleep1(unsigned int ms)
{
    if (ms <= 0) {
...
hi_void IotPublishSample(void)
{
    ...
    WeChatProfile weChatProfile = {
        ...
        .reportAction.motorActionStatus = getWeight(), /* 0 : motor off */
        /* report temperature */
        .reportAction.subDeviceActionTemperature = "temperature",
        .reportAction.temperatureData = getTransTemp(), /* 30 :temperature data */
        /* report humidity */
        .reportAction.subDeviceActionHumidity = "humidity",
        .reportAction.humidityActionData = getTransHum(), /* humidity data */
        /* report light_intensity */
        .reportAction.subDeviceActionLightIntensity = "light_intensity",
        .reportAction.lightIntensityActionData = GetTemp(), /* 60 : 预设温度 */
        /* report fruit */
        .reportAction.subDeviceActionLightIntensity = "fruit",
        .reportAction.lightIntensityActionData = getFruit(), /* 0 : 无食物，有数字则有食物 */
    };
...
static hi_void *DemoEntry(const char *arg)
{
    WifiStaReadyWait();
    cJsonInit();
    IoTMain();  //zzx：这个方法里有一个线程，里面是mqtt连接服务器的初始化方法
    printf("[zzx]iot initial success!\n");
    //zzx:问题出在这之前
    /* 云端下发回调 */
    IoTSetMsgCallback(DemoMsgRcvCallBack);
    hi_sleep(50000);  //留足连接mqtt的时间
    printf("[zzx] board begin send message\n");
    /* 主动上报 */
...
static void AppDemoIot(void)
{
    ...
    attr.name = "IOTDEMO";
    attr.attr_bits = 0U;  //0U
    attr.cb_mem = NULL;  // NULL
    attr.cb_size = 0U;  //0U
    attr.stack_mem = NULL; //NULL
    attr.stack_size = CN_IOT_TASK_STACKSIZE * 10;
