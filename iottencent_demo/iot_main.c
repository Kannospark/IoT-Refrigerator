...
static const char *gDefaultSubscribeTopic[] = {
    ...
    "KPVVQJOAHL/Hi3861_mqtt/data",
    "KPVVQJOAHL/Hi3861_mqtt/event",
    "KPVVQJOAHL/Hi3861_mqtt/control",
};
...
static int MsgRcvCallBack(char *context, char *topic, int topicLen, MQTTClient_message *message)
{
    ...
    if (buf != NULL) {
        ...
        if (IOT_SUCCESS != osMessageQueuePut(gIoTAppCb.queueID, &msg, 0, CN_QUEUE_WAITTIMEOUT)) {
            ...
            hi_free(0, msg);
            // printf("[zzx] release memory successfully\n");
        }
        // printf("[zzx] forget to release meory once\n");
    }
...
static int ProcessQueueMsg(MQTTClient client)
{
    ...
    do {
        ...
        if (msg != NULL) {
            // printf("check3.5.4\n");
            //IOT_LOG_DEBUG("QUEUEMSG:QOS:%d TOPIC:%s PAYLOAD:%s\r\n", msg->qos, msg->topic, msg->payload);  //zzx：这里不注释会stack error
            // printf("check3.5.5\n");
...
static void MainEntryProcess(void)
{
    ...
    // printf("[zzx] get in MainEntryProcess\n");
    ...
    // printf("[zzx] check3\n");
    rc = MQTTClient_connect(client, &connOpts);  //zzx:这个方法大有问题！
    if (MqttDestory(rc, client) != MQTTCLIENT_SUCCESS) {
        // printf("[zzx] error 3\n");
        return;
    }
    // printf("[zzx] now arrived CN_TOPIC_SUBSCRIBE_NUM, CN_TOPIC_SUBSCRIBE_NUM is %d\n",CN_TOPIC_SUBSCRIBE_NUM);
...
int IoTMain(void)
{
    ...
    attr.stack_size = CN_TASK_STACKSIZE * 4;
...
int IotSendMsg(int qos, const char *topic, const char *payload)
{
    ...
    if (buf != NULL) {
        ...
        if (IOT_SUCCESS != osMessageQueuePut(gIoTAppCb.queueID, &msg, 0, CN_QUEUE_WAITTIMEOUT)) {
            ...
            rc = 0;
            hi_free(0, msg);  //zzx:手动释放
            // printf("[zzx] fail to release memory\n");
        }
    }
    return rc;
}
