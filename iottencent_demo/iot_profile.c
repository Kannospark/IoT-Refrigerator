...
static char *MakeProfileReport(WeChatProfile *payload)
{
    ...
    cJSON_AddNumberToObject(reported, payload->reportAction.subDeviceActionFruit,
                            payload->reportAction.fruitData);
    ret = cJSON_PrintUnformatted(root);
    cJSON_Delete(state);
...
#define CN_PROFILE_TOPICFMT_TOPIC            "$shadow/operation/KPVVQJOAHL/Hi3861_mqtt"
