...
void Uart1GpioInit(void)
{
    ...
    IoSetFunc2(IOT_IO_NAME_GPIO_0, IOT_IO_FUNC_GPIO_0_UART1_TXD);
    IoTGpioInit(IOT_IO_NAME_GPIO_1);
    // 设置GPIO1的管脚复用关系为UART1_RX Set the pin reuse relationship of GPIO1 to UART1_ RX
    IoSetFunc2(IOT_IO_NAME_GPIO_1, IOT_IO_FUNC_GPIO_1_UART1_RXD);
}
...
static void UartTask(void)
{
    ...
    while (1) {
        ...
        IoTUartWrite(HI_UART_IDX_1, &fruit, 4);
        // 通过UART1 接收数据 Receive data through UART1
        len = IoTUartRead(HI_UART_IDX_1, &uartReadBuff, 4);
        if (len > 0) {
            // 把接收到的数据打印出来 Print the received data
            printf("Uart Read Data is: [ %d ] %d \r\n", count, uartReadBuff);
        }
        usleep(10000);
        count++;
    }
...
void UartExampleEntry(void)
{
    ...
    attr.priority = osPriorityNormal + 10;
